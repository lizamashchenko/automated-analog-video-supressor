"""
Detector — runs the full-spectrum FPV detection pipeline in a background thread.

Accepts a config dict and an optional on_event(event_type, data) callback.
The callback is called from the detector thread; it must be thread-safe.

Event types emitted:
  status          {state: "running"|"stopped", run_name?: str}
  sweep_start     {sweep_num, min_freq, max_freq}
  freq_update     {freq, progress}          -- 0.0–1.0
  spectrum        {freq, power}             -- list of ~256 floats (dB, downsampled)
  plateau_confirmed  {freq, bandwidth, hits}
  plateau_rejected   {freq, hits, required}
  video_confirmed    {freq, classifier, score}
  video_rejected     {freq, classifier, score}
  error           {error_type, message, freq?}
  sweep_complete  {sweep_num, plateaus}
"""
...
class Detector:
    ...
    def start(self, run_name=None):
        ...
        self._thread = threading.Thread(
            target=self._run, args=(run_name,), daemon=True
        )
        self._thread.start()
    ...
    def _sweep(self, sweep_num, min_freq, max_freq):
        ...
        while current_freq < max_freq and not self._stop.is_set():
            ...
            for _ in range(wide_sampling_num):
                ...
                if samples is None:
                    self.log.log_event("ZERO_BUFFER", "All samples are zero",
                                       level=1, freq=current_freq)
                    self._emit("error", {"error_type": "ZERO_BUFFER",
                                         "message": "Zero buffer", "freq": current_freq})
                    continue
            ...
            # emit downsampled spectrum for this step
            if state.avg_power is not None:
                self._emit("spectrum", {
                    "freq": current_freq,
                    "power": _downsample(state.avg_power, 256)
                })
            ...
            if not plateau:
                if len(detections) > 0:
                    self.log.log_event("PLATEAU_REJECTED", "Not enough hits", level=2,
                                       hits=len(detections), required=required_hits,
                                       freq=current_freq)
                    self._emit("plateau_rejected", {
                        "freq": current_freq,
                        "hits": len(detections),
            ...
            self.log.log_confirmed_plateau(
                plateau["center_freq"], plateau["bandwidth"], len(detections))
            self.log.log_event("PLATEAU_CONFIRMED", "Wideband signal detected", level=2,
                               freq=plateau["center_freq"], bw=plateau["bandwidth"],
                               hits=len(detections))
        ...
        for key, samples_list in state.plateau_map.items():
            ...
            if result["confirmed"]:
                ...
                self.log.log_event("VIDEO_CONFIRMED", f"{self.classifier.name} confirmed video",
                                   level=2, freq=center_freq, score=result["score"])
                ...
                self.log.log_event("VIDEO_REJECTED", f"{self.classifier.name} rejected video",
                                   level=2, freq=center_freq, score=result["score"])
